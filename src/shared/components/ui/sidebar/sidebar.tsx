...
type SidebarTriggerProps = Omit<
  ...
  'buttonVariant' | 'iconOnly' | 'title' | 'variant'
>;

export function SidebarTrigger({ className, onClick, ...props }: SidebarTriggerProps) {
  const { toggleSidebar } = useSidebar();

  // Button's onClick type is an intersection of BaseUI's event-based
  // handler and TButtonProps' icon-variant `() => void`, which no single
  // real handler can satisfy. Widen locally until TButtonProps is fixed
  // upstream to type onClick with the event for the 'icon' variant too.
  const handleTriggerClick = (...args: unknown[]) => {
    (onClick as ((...args: unknown[]) => void) | undefined)?.(...args);
    toggleSidebar();
  };
