...
function FieldGroup({
  ...
  variant = 'default',
  ...props
}: React.ComponentProps<'div'> & { variant?: 'default' | 'checkbox' }) {
  return (
    <div
      data-slot="field-group"
      className={cn(
        ' group/field-group @container/field-group flex w-full flex-col gap-7 data-[slot=checkbox-group]:gap-3 *:data-[slot=field-group]:gap-4',
        variant === 'checkbox' && 'gap-2.5',
        className
      )}
      {...props}
    />
...
const fieldVariants = cva('group/field flex w-full gap-3 data-[invalid=true]:text-destructive', {
  variants: {
    orientation: {
      ...
      horizontal:
        'gap-2.5 flex-row items-center has-[>[data-slot=field-content]]:items-start *:data-[slot=field-label]:flex-auto has-[>[data-slot=field-content]]:[&>[role=checkbox],[role=radio]]:mt-px',
      responsive:
