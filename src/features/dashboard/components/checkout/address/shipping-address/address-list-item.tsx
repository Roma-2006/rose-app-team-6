  ...
  return (
    ...
    >
      <div className="flex flex-col gap-1.5">
        <h2 className={cn('font-semibold text-2xl', isSelected ? 'text-text-inverse' : 'text-text-plain')}>
          {address.city}
        </h2>
        <p
          className={cn(
      ...
      {address.phone && (
        <div className="flex gap-1.5 items-center shrink-0 self-start ">
          ...
          <p className={cn('text-lg font-medium', isSelected ? 'text-text-subtle' : 'text-text-default')}>
            {address.phone}
          </p>
        </div>
      )}
