...
export default function AccountSidebar({ activeTab, onLogout }: AccountSidebarProps) {
  ...
  return (
    <nav
      className="fixed bottom-0 left-0 z-50   lg:static lg:z-0 w-full lg:w-[25%] h-auto lg:h-158.5 border border-border-muted p-2 lg:p-4 flex flex-row lg:flex-col justify-between rounded-xl shadow-sm bg-bg-plain select-none"
    ...
    >
      ...
      <div className="w-auto lg:w-full">
        <Button
          ...
          onClick={onLogout}
          className=" w-full justify-center lg:justify-start gap-2 lg:gap-3 px-2 lg:px-4  py-2.5 lg:py-3.5  rounded-lg lg:rounded-xl  transition-all text-xs sm:text-sm lg:text-lg font-medium bg-bg-muted text-text-danger border-none"
        />
      </div>
    </nav>
  );
